...
use tauri::{async_runtime, AppHandle, Emitter, Listener, Manager, Runtime, State};
...
pub async fn stop_capturing<R: Runtime>(
    ...
    handle.emit(&thread_id, ()).expect("emit stop event");
...
fn capture_thread<R: Runtime>(handle: AppHandle<R>, thread_id: Uuid, cap: Capture<Active>) {
    ...
    handle.listen_any(thread_id.to_string(), move |_| {
