...
impl Node {
    fn new(ip: Vec<IpAddr>, latitude: f64, longitude: f64) -> Self {
        Node {
            ip,
            latitude,
            longitude,
...
async fn try_add_ip(
    ...
    let new_node: Node = Node::new(vec![ip], lat, lon); 
    match nodes.iter().position(|x| *x == new_node ) {
        Some(i) => {
            nodes[i].ip.push(ip);
            println!("{}, {} has already been added", lat, lon);
        },
        None => {
            nodes.push(new_node);
            ...
            IP_JSON_DOCUMENT.write().unwrap().push_str(&format!(r#"{{"ip":"{}","lat":"{}","lon":"{}","city":"{}",}},"#,ip, lat, lon, city));
