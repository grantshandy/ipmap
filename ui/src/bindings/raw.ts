...
export const commands = {
  ...
  async loadDatabase(
    path: string,
    err: TAURI_CHANNEL<string>,
  ): Promise<Result<null, string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("load_database", { path, err }),
      };
    } catch (e) {
  ...
  async startCapture(
    device: Device,
    connectionChannel: TAURI_CHANNEL<Connections>,
  ): Promise<Result<null, string>> {
    try {
      return {
        status: "ok",
        data: await TAURI_INVOKE("start_capture", {
          device,
          connectionChannel,
        }),
      };
